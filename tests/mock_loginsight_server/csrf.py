...
class MockedCsrfMixin(object):

    def __init__(self, **kwargs):
        super(MockedCsrfMixin, self).__init__(**kwargs)

        self.sessions_known = RandomDict()
        self.users_known = [User('admin', 'password', 'mock', "012345678-9ab-cdef-0123-456789abcdef")]

        self.register_uri('GET', '/csrf', text=self.csrf_new, status_code=200)

    def csrf_new(self, request, context):
        assert request.headers[CSRFHEADER] == 'Fetch'
