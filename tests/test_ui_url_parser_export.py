...
def explode_existingMessageQuery(list_of_json_strings):
    assert len(list_of_json_strings) == 1
    return json.loads(list_of_json_strings[0])


def explode_url_components(url, expected_different_fields=None):
    """Helper function consumes a url, with or without scheme/hostname, and returns a dictionary for the query-component"""

    if expected_different_fields is None:
        expected_different_fields = ["paramsHelper.clientNowMillis", "downloadToken", "exportHelper.exportToken"]
    ...
    for queryparam in d:
        print(queryparam)
        if queryparam in expected_different_fields:  # These fields are going to be different for every request, but we want to know that it was present.
            r[queryparam] = "ignored.{0}".format(queryparam)  # Set to a dummy value containing the field name
        elif queryparam in ['existingMessageQuery']:
            r[queryparam] = explode_existingMessageQuery(d[queryparam])
        else:
            r[queryparam] = d[queryparam]
    return r
